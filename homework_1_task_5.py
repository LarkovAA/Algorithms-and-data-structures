...
def vchecking_distance(a, b):
    if set_alphabet.get(a) > set_alphabet.get(b):
        return set_alphabet.get(a) - set_alphabet.get(b)
    else:
        return set_alphabet.get(b) - set_alphabet.get(b)
...
print(f'Данные буквы находятся на местах {a} = {set_alphabet[a]}, {b} = {set_alphabet[b]}, расстояние междду ними {vchecking_distance(a, b)}')